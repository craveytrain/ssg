...
var buildModel = function(page, path) {
	...
	var pageData = jsYAML.load(fileContents[1]);
	pageData.body = marked(fileContents[2]);

	var pageModel = extend(true, {}, baseModel, pageData);
	...
	siteModel.posts.push(pageModel);
...
var buildJadeData = function() {
	...
	content.forEach(function(glob) {
		grunt.file.expandFiles(glob).forEach(function(file) {
			...
			jadeData[controller].files[target + controller + 'index.html'] = templates + path.sep + jadeData[controller].options.data.page.pageType + '.jade';
